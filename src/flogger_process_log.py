...
# 
# 20181218            Add code to check take-off within 5km of airfield.
# 
...
def process_log (cursor, db, settings):
    ...
    #
    # This determines if the take off point was on the airfield, ie within 2km radius
    #
    #    track table is id, latitude, longitude, altitude, flight_no
    #    flight_log_final table has flight_no for this flight, ie in row
    #    This selects the first flight table record's coords in the flights track file, ie the take off point
        cursor.execute("SELECT latitude, longitude FROM track WHERE flight_no=? LIMIT 1", (row[9],))
        ...
        dist =  geopy.distance.vincenty(take_off_coords, airfield_coords).km
        if dist > 2:
        ...
        if strt_date >= max_date:
            ...
            delta_alt = (float(row[6]) - (int(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN)))
            ...
            if duration > MINTIME:
                ...
                print "Row copied"
#            else:
            elif (float(row[6]) - (int(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))) <= 0:
...
#                if row[6] <= (settings.FLOGGER_QNH + settings.FLOGGER_QFE_MIN):
                print "====Ignore row, flight time too short and too low. Time: ", row[4], " alt: ", row[6], " QNH Min: ", (int(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))
#                else:
            elif (float(row[6]) - (int(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))) > 0:
                print "++++Accept row, short flight but ok min height. Time: ", row[4], " alt: ", row[6], " QNH Min: ", (int(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))
                cursor.execute('''INSERT INTO flight_log(sdate, stime, edate, etime, duration, src_callsign, max_altitude, speed, registration,flight_no)
                                VALUES(:sdate,:stime,:edate,:etime,:duration,:src_callsign,:max_altitude,:speed, :registration,:flight_no)''',
                                {'sdate':row[0], 'stime':row[1], 'edate': row[2], 'etime':row[3],
                                'duration': row[4], 'src_callsign':row[5], 'max_altitude':row[6], 'speed':row[7], 'registration':row[8], 'flight_no':row[9]})
